...
def scope_next_hard_block(
    ...
    *,
    ...
    while start < length:
        ...
        scoped = torch.zeros_like(mask, dtype=torch.bool)
...
def compute_window_pressure(
    ...
    if config.enable_g_gate:
        from .selector_cd_apc import position_gate_mask
...
def update_ccaw_state(
    ...
    """Update persistent capacity after one atomic commit (legacy expand-on-pressure)."""
...
def update_inverse_ccaw_state(
    ...
    current_pressure = min(
        ...
        max(
            0.0,
            float(pressure.combined) * float(config.ccaw_pressure_scale),
        ),
    )
    target = maximum - round((maximum - minimum) * current_pressure)
    difference = target - int(state.mask_capacity)
    difference = max(
        -int(config.ccaw_shrink_step),
        min(int(config.ccaw_expand_step), difference),
    ...
    state.mask_capacity = max(
        minimum, min(maximum, int(state.mask_capacity) + difference)
    )
...
def pressure_adaptive_commit_budget(
    ...
    budget = round(maximum - (maximum - minimum) * pressure_value)
