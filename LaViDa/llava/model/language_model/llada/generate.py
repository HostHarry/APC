...
def generate(model, prompt=None, steps=None, max_new_tokens=128, block_length=128, temperature=0.,
    ...
    thinking_config = resolve_thinking_config(thinking_config)
    ...
    if thinking_config.vrg_enabled:
        if cfg_scale > 0.0:
            raise ValueError("VRG cannot be combined with cfg_scale > 0")
        if inputs_embeds is None or visual_mask is None:
            raise ValueError(
                "VRG requires multimodal inputs_embeds and a visual_mask"
            )
    if prefix_lm:
        if thinking_config.vrg_enabled:
            past_key_values = visual_guided_prefill(
                model,
                inputs_embeds,
                visual_mask,
                attention_mask=attention_mask,
                force_math_sdpa=thinking_config.force_math_sdpa,
            )
        else:
    ...
    response_start = int(x.shape[1] - gen_length)
    response_end = int(x.shape[1])
    swd_state = SwdState() if thinking_config.swd_enabled else None
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            if cfg_scale > 0.:
                ...
                if thinking_config.vrg_enabled:
                    logits = visual_guided_logits(
                        ...
                        inputs_embeds_curr,
                        visual_mask,
                        scale=thinking_config.vrg_scale,
                        attention_mask=attention_mask,
                        force_math_sdpa=thinking_config.force_math_sdpa,
            ...
            if remasking == 'low_confidence':
                if thinking_config.swd_enabled:
                    x0_p, swd_probs = response_span_token_probs(
                        logits,
                        x0,
            ...
            if thinking_config.psp_enabled and remasking == 'low_confidence':
                confidence = apply_psp(
                    ...
                    response_start=response_start,
                    response_end=response_end,
                    gamma=thinking_config.psp_gamma,
                )
            if (
                thinking_config.swd_enabled
                and remasking == 'low_confidence'
                and swd_state is not None
            ):
                confidence = apply_swd(
                    confidence,
                    logits,
                    swd_state,
                    lambda_=thinking_config.swd_lambda,
                    response_start=response_start,
                    response_end=response_end,
                    mask_index=mask_index,
                    current_probs=swd_probs,
                )
