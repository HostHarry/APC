...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            ...
            # MDM attention is non-causal by default, but callers may still
            # provide an additive mask for padding, prefix-LM, or visual
            # ablation.
    ...
    def attention(
        ...
        # shape: (B, nh, T, hs)
        # Prefer SDPA whenever an additive attention_bias is present. The flex
        # path only consumes block_mask and would otherwise silently drop
        # visual-ablation / prefix-LM response masks.
        if block_mask is not None and attention_bias is None:
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            ...
            raise NotImplementedError("Block group size > 1 is not yet supported in LLaDA")
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if num_items_in_batch is None:
                ...
                token_loss = F.cross_entropy(logits.flatten(0,1),labels.flatten(),reduction='sum')
                token_loss = token_loss / num_items_in_batch
                # print(token_loss)
            # tested, equiivalent to 
            # token_loss = F.cross_entropy(logits.flatten(0,1),labels.view(-1),reduction='none')
            # token_loss2 = token_loss.sum() / (labels_mask.sum())
            # import warnings
            loss = token_loss
