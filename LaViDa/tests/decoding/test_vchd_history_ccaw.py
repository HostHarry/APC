...
from llava.decoding.selector import (
...
def test_history_anchor_deferral_stays_inside_hard_block():
    """Anchor deferral must not pick a token outside the current hard block."""
    ...
    first = report["trace"][0]
    assert first["hard_block_left"] == 0
    assert first["hard_block_right"] == 1
    assert all(
        first["hard_block_left"] <= position < first["hard_block_right"]
        for position in first["selected_positions"]
    )


def test_g_gate_block_rate_is_counted():
    mask_id = 4
    tokens = torch.tensor([[0, 2, 3, mask_id, mask_id, mask_id]])
    config = VCHDDecodeConfig(
        mask_id=mask_id,
    ...
    assert report["enable_g_gate"]
    # FixedModel ignores attention bias, so visual gain ~0 and always_pos
    # blocks every u/r-passing position → block rate must be 1.0.
    assert report["g_gate_block_rate"] == 1.0


def test_g_gate_block_rate_respects_disabled_u_gate():
    """Disabled u-gate must not filter ur_pass via tau_base (ablation case)."""
    ...
    config = VCHDDecodeConfig(
        ...
        text_vocab_size=5,
        forbidden_token_ids=(0, 3, mask_id),
        # FixedModel base conf ~0.997; tau_base=1.0 empties ur_pass if the
        # disabled u-gate switch is ignored → old bug reported rate 0.0.
        tau_base=1.0,
        tau_contrast=0.0,
        enable_u_gate=False,
        enable_r_gate=True,
    ...
    assert report["enable_u_gate"] is False
    assert report["enable_g_gate"]
    assert report["g_gate_block_rate"] == 1.0


def test_g_gate_block_rate_respects_disabled_r_gate():
    """Disabled r-gate must not filter ur_pass via tau_contrast."""
    ...
    assert report["enable_r_gate"] is False
    assert report["enable_g_gate"]
    assert report["g_gate_block_rate"] == 1.0
