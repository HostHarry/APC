...
from models.mmada_decode import MMaDADecodeConfig, dispatch_dcd_decode_text
...
def generate_with_prefix_cache(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    cfg = MMaDADecodeConfig(
        ...
        mask_id=mask_id,
        cache_type="prefix",
        decode_algo="threshold" if threshold is not None else "factor",
        decode_param=threshold if threshold is not None else 1.0 / max(1, steps),
    )
    out = dispatch_dcd_decode_text(model, x, prompt.shape[1], prompt.shape[1] + gen_length, cfg, attention_bias, prompt_index)
    return out if not cfg.debug else out[0]
