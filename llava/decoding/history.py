...
class SparseHistory:
    ...
    last_top1_token: int
    consecutive_top1_matches: int = 0
...
def _ema_history(
    ...
    *,
    ...
    return SparseHistory(
        ...
        consecutive_top1_matches=int(history.consecutive_top1_matches),
    )
...
def observe_sparse_history(
    ...
    *,
    ...
    current_top1 = int(current_ids[0].item())
    ...
    if current_top1 == int(history.last_top1_token):
        matches = int(history.consecutive_top1_matches) + 1
    else:
        matches = 0
    ...
    next_history = SparseHistory(
        token_ids=next_history.token_ids,
        probs=next_history.probs,
        other_prob=next_history.other_prob,
        last_observed_context_version=next_history.last_observed_context_version,
        last_top1_token=current_top1,
        consecutive_top1_matches=matches,
    )
    return HistoryObservation(
        stability=stability,
        next_history=next_history,
...
def history_adjusted_reliability(
    ...
    """R = C_contrast * [1 - penalty_scale * rho * (1 - T)]."""
    ...
    if penalty_scale < 0.0:
        raise ValueError(f"penalty_scale must be non-negative, got {penalty_scale}")
