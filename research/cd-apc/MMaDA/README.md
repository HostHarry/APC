...
### History-VCHD decoding

The MMU generation entry point also accepts `decode_strategy="vchd"` with a
`decoding.VCHDDecodeConfig`. This training-free path combines a
visual/visual-access-ablated paired forward, dual confidence gates, optional
sparse trajectory history, and an optional conflict-adaptive window (CCAW).
The VLMEvalKit wrapper exposes these settings through `MMADA_VCHD_*`
environment variables; `VLind-Bench/eval/mmada_eval.py` provides the
corresponding command-line flags and component-ablation profiles.
