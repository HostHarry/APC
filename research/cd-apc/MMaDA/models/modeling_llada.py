...
class RotaryEmbedding(nn.Module):
    ...
    def forward(
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            rope_len = block_end_index if block_end_index is not None else key_len
            pos_sin, pos_cos = self.get_rotary_embedding(rope_len, q_.device)
            pos_sin = pos_sin.type_as(q_)
            pos_cos = pos_cos.type_as(q_)
            q_ = self.apply_rotary_pos_emb(
                pos_sin[:, :, rope_len - query_len : rope_len, :],
                pos_cos[:, :, rope_len - query_len : rope_len, :],
                q_,
            )
            k_ = self.apply_rotary_pos_emb(pos_sin[:, :, rope_len - key_len : rope_len, :], pos_cos[:, :, rope_len - key_len : rope_len, :], k_)
    ...
    def attention(
        ...
        if layer_past is not None:
            ...
            if replace_position is None:
                ...
                n_replace = int(replace_position.sum(dim=-1).max().item()) if replace_position.numel() else 0
                assert n_replace <= T, "replace_position selects more tokens than provided in the current input"
                for batch_idx in range(replace_position.shape[0]):
                    ...
                    if len(batch_replace_indices) > 0:
                        replace_len = len(batch_replace_indices)
                        past_key[batch_idx, :, batch_replace_indices] = k[batch_idx, :, :replace_len]
                        past_value[batch_idx, :, batch_replace_indices] = v[batch_idx, :, :replace_len]
                k = past_key
                v = past_value
        ...
        if self.config.rope:
            if replace_position is not None and key_len != query_len:
                nonzero = replace_position.nonzero(as_tuple=True)
                block_end = int(nonzero[1].max().item()) + 1 if len(nonzero[1]) > 0 else key_len
                q, _ = self.rotary_emb(q, q, block_end)
                _, k = self.rotary_emb(k, k, key_len)
            else:
                q, k = self.rotary_emb(q, k, None)
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache = self.attention(
                q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, replace_position=replace_position
            )
    ...
    def forward(
        ...
        for block_idx, block in enumerate(self):
            ...
            if (
                ...
                # shape: (batch_size, seq_len, d_model)
                x, cache = block(
                    x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, replace_position=replace_position
                )
    ...
    def forward(
        ...
        if (
            ...
            if attention_mask is not None:
                ...
                mask_len = past_key_values[0][0].shape[-2] + seq_len if kv_grows else past_key_values[0][0].shape[-2]
            if attention_bias is not None:
        ...
        if self.config.block_group_size == 1:
            ...
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, cache = block_group(
                    x, attention_bias=attention_bias, layers_past=layers_past, use_cache=use_cache, replace_position=replace_position
                )
                if attn_key_values is not None:
                    assert cache is not None
